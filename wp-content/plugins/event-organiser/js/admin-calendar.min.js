(function(b){function a(c){var d=new Date(NaN),h=/^\s*(\d{4})-(\d\d)-(\d\d)\s*$/.exec(c);if(h){c=+h[2];d.setFullYear(h[1],c-1,h[3]);c!=d.getMonth()+1&&d.setTime(NaN)}return d}b(document).ready(function(){function c(){return b("<span class='fc-header-goto'><input type='hidden' id='miniCalendar'/></span>")}function d(e){var f=e.categories,j="<select class='eo-cal-filter' id='eo-event-cat'>";j+="<option value=''>"+e.buttonText.cat+"</option>";for(e=0;e<f.length;e++)j+="<option class='cat-slug-"+f[e].slug+
" cat' value='"+f[e].slug+"'>"+f[e].name+"</option>";j+="</select>";return b("<span class='fc-header-dropdown filter-category'></span>").append(j)}function h(e){var f=e.venues,j="<select class='eo-cal-filter' id='eo-event-venue'>";j+="<option value=''>"+e.buttonText.venue+"</option>";for(e=0;e<f.length;e++)j+="<option value='"+f[e].term_id+"'>"+f[e].name+"</option>";j+="</select>";return b("<span class='fc-header-dropdown filter-venue'></span>").append(j)}b("#eo-dialog-tabs").tabs();b(".eo-dialog").dialog({autoOpen:false,
width:527,modal:true});b("#events-meta").parent().find(".ui-dialog-titlebar-close").appendTo(".ui-tabs-nav").closest(".ui-dialog").children(".ui-dialog-titlebar").remove();var g=b("#eofc_time_format").is(":checked")?"HH:mm":"h:mmtt",i=a(b.cookie("eo_admin_cal_last_viewed_date"));if(Object.prototype.toString.call(i)==="[object Date]"){if(isNaN(i.getTime()))i=new Date}else i=new Date;var k=jQuery("#eo_admin_calendar").fullCalendar({firstDay:parseInt(EO_Ajax.startday),date:i.getDate(),month:i.getMonth(),
year:i.getFullYear(),defaultView:b.cookie("eo_admin_cal_last_view")?b.cookie("eo_admin_cal_last_view"):"month",editable:false,lazyFetching:"true",eventColor:"#21759B",theme:true,customButtons:{category:d,venue:h,"goto":c},buttonText:{today:EO_Ajax.locale.today,month:EO_Ajax.locale.month,week:EO_Ajax.locale.week,day:EO_Ajax.locale.day,cat:EO_Ajax.locale.cat,venue:EO_Ajax.locale.venue},monthNames:EO_Ajax.locale.monthNames,monthNamesShort:EO_Ajax.locale.monthAbbrev,dayNames:EO_Ajax.locale.dayNames,dayNamesShort:EO_Ajax.locale.dayAbbrev,
header:{left:"title",center:"category venue",right:"prev goto today next"},buttonIcons:false,buttonui:true,events:function(e,f,j){jQuery.ajax({url:EO_Ajax.ajaxurl+"?action=event-admin-cal",dataType:"JSON",data:{start:jQuery.fullCalendar.formatDate(e,"yyyy-MM-dd"),end:jQuery.fullCalendar.formatDate(f,"yyyy-MM-dd")},success:function(l){j(l)}})},categories:EO_Ajax.categories,venues:EO_Ajax.venues,selectable:true,selectHelper:true,eventRender:function(e){var f=jQuery(".filter-category .eo-cal-filter").val(),
j=jQuery(".filter-venue .eo-cal-filter").val();if(typeof f!=="undefined"&&f!=""&&jQuery.inArray(f,e.category)<0)return"<div></div>";if(typeof j!=="undefined"&&j!=""&&j!=e.venue)return"<div></div>"},viewDisplay:function(e){var f=jQuery.fullCalendar.formatDate(e.start,"yyyy-MM-dd");e=e.name;var j=new Date;j=new Date(j.getTime()+6E5);b.cookie("eo_admin_cal_last_viewed_date",f,{expires:j});b.cookie("eo_admin_cal_last_view",e,{expires:j})},weekMode:"variable",aspectRatio:1.5,loading:function(e){e?jQuery("#loading").show():
jQuery("#loading").hide()},timeFormat:g,axisFormat:g,eventClick:function(e,f){f.preventDefault();jQuery("#eo-dialog-tabs ul li").each(function(){var j=b(this).attr("id").substring(14);jQuery("#eo-dialog-tabs #"+b(this).attr("id")+"-content").html(e[j])});b("#events-meta").dialog("open")},select:function(e,f,j,l){if(EO_Ajax.perm_edit){l.preventDefault();l=jQuery(this)[0].calendar.options;var n=jQuery.fullCalendar.formatDate(e,"yyyy-MM-dd"),m=jQuery.fullCalendar.formatDate(e,"HH:mm"),o=jQuery.fullCalendar.formatDate(f,
"yyyy-MM-dd"),p=jQuery.fullCalendar.formatDate(f,"HH:mm");if(j){g="ddd, dS MMMM";j=1}else{g="ddd, dS MMMM h(:mm)tt";j=0}if(n==o){e=jQuery.fullCalendar.formatDate(e,g,l);j||(e=e+" &mdash; "+jQuery.fullCalendar.formatDate(f,"h(:mm)tt",l))}else e=jQuery.fullCalendar.formatDate(e,g,l)+" &mdash; "+jQuery.fullCalendar.formatDate(f,g,l);b("#eo_event_create_cal input[name='eo_event[event_title]']").val("");b("#eo_event_create_cal input.ui-autocomplete-input").val("");b("#eo_event_create_cal textarea[name='eo_event[event_content]']").val("");
b("#eo_event_create_cal input[name='eo_event[StartDate]']").val(n);b("#eo_event_create_cal input[name='eo_event[StartTime]']").val(m);b("#eo_event_create_cal input[name='eo_event[EndDate]']").val(o);b("#eo_event_create_cal input[name='eo_event[FinishTime]']").val(p);b("#eo_event_create_cal input[name='eo_event[allday]']").val(j);b("#eo_event_create_cal td#date").html(e);b("#eo_event_create_cal").dialog("open");b("form.eo_cal input[type='submit']").removeAttr("disabled");b("form.eo_cal input#reset").click(function(){b("#eo_event_create_cal").dialog("close")})}}});
b("#eofc_time_format").change(function(){g=b("#eofc_time_format").is(":checked")?"HH:mm":"h:mmtt";k.fullCalendar("option","timeFormat",g);b.post(ajaxurl,{action:"eofc-format-time",is24:b("#eofc_time_format").is(":checked")})});b(".view-button").click(function(e){e.preventDefault();b(".view-button").removeClass("active");k.fullCalendar("changeView",b(this).attr("id"));b(this).addClass("active")});b("#miniCalendar").datepicker({dateFormat:"DD, d MM, yy",firstDay:parseInt(EO_Ajax.startday),changeMonth:true,
monthNamesShort:EO_Ajax.locale.monthAbbrev,dayNamesMin:EO_Ajax.locale.dayAbbrev,changeYear:true,showOn:"button",buttonText:EO_Ajax.locale.gotodate,onSelect:function(e){k.fullCalendar("gotoDate",new Date(Date.parse(e)))}});b("button.ui-datepicker-trigger").button();b.widget("ui.combobox",{_create:function(){var e=this.element.hide(),f=e.children(":selected");f=f.val()?f.text():"";e=b("<span>").addClass("ui-combobox eo-venue-input").insertAfter(e);var j={delay:0,minLength:0,source:function(n,m){b.getJSON(EO_Ajax.ajaxurl+
"?callback=?&action=eo-search-venue",n,function(o){o=b.map(o,function(p){p.label=p.name;return p});m(o)})},select:function(n,m){b("#venue_select").removeAttr("selected");b("#venue_select").val(m.item.term_id)}},l=b("<input>").appendTo(e).val(f).addClass("ui-combobox-input").autocomplete(j).addClass("ui-widget-content ui-corner-left");l.data("autocomplete")._renderItem=function(n,m){if(m.term_id==0)return b("<li></li>").data("item.autocomplete",m).append("<a>"+m.label+"</a>").appendTo(n);var o=b.grep([m.venue_address,
m.venue_city,m.venue_state,m.venue_postcode,m.venue_country],function(p){return p}).join(", ");return b("<li></li>").data("item.autocomplete",m).append("<a>"+m.label+"</br> <span style='font-size: 0.8em'><em>"+o+"</span></em></a>").appendTo(n)};f=b("<span>").addClass("eo-venue-combobox-buttons").appendTo(e);b("<a style='vertical-align: top;margin: 0px -1px;padding: 0px;height: 21px;'>").attr("title","Show All Items").appendTo(f).button({icons:{primary:"ui-icon-triangle-1-s"},text:false}).removeClass("ui-corner-all").addClass("ui-corner-right ui-combobox-toggle ui-combobox-button").click(function(){if(l.autocomplete("widget").is(":visible"))l.autocomplete("close");
else{b(this).blur();l.autocomplete("search","").focus()}})}});b("#venue_select").combobox();b(".eo-cal-filter").change(function(){k.fullCalendar("rerenderEvents")});b(".filter-venue .eo-cal-filter").selectmenu({wrapperElement:"<span class='fc-header-filter'></span>"});b(".filter-category .eo-cal-filter").selectmenu({wrapperElement:"<span class='fc-header-filter'></span>",icons:[{find:".cat"}]});i=b("#eo-event-venue-button").width()+30;b("#eo-event-venue-button").width(i+"px");b("#eo-event-venue-menu").width(i+
"px");i=b("#eo-event-cat-button").width()+30;b("#eo-event-cat-button").width(i+"px");b("#eo-event-cat-menu").width(i+"px")})})(jQuery);
...
c=this.options,d=[];this.element.find("option").each(function(){var f=b(this);d.push({value:f.attr("value"),text:a._formatText(f.text(),f),selected:f.attr("selected"),disabled:f.attr("disabled"),classes:f.attr("class"),typeahead:f.attr("typeahead"),parentOptGroup:f.parent("optgroup"),bgImage:c.bgImage.call(f)})});var h=a.options.style=="popup"?" ui-state-active":"";this.list.html("");if(d.length)for(var g=0;g<d.length;g++){var i={role:"presentation"};if(d[g].disabled)i["class"]=this.namespace+"-state-disabled";
var k={html:d[g].text||"&nbsp;",href:"#nogo",tabindex:-1,role:"option","aria-selected":false};if(d[g].disabled)k["aria-disabled"]=d[g].disabled;if(d[g].typeahead)k.typeahead=d[g].typeahead;k=b("<a/>",k).bind("focus.selectmenu",function(){b(this).parent().mouseover()}).bind("blur.selectmenu",function(){b(this).parent().mouseout()});i=b("<li/>",i).append(k).data("index",g).addClass(d[g].classes).data("optionClasses",d[g].classes||"").bind("mouseup.selectmenu",function(f){if(a._safemouseup&&!a._disabled(f.currentTarget)&&
...
"-item-focus ui-state-hover")}}).bind("mouseout.selectmenu",function(){b(this).is(a._selectedOptionLi())&&b(this).addClass(h);b(this).removeClass(a.widgetBaseClass+"-item-focus ui-state-hover")});if(d[g].parentOptGroup.length){k=a.widgetBaseClass+"-group-"+this.element.find("optgroup").index(d[g].parentOptGroup);this.list.find("li."+k).length?this.list.find("li."+k+":last ul").append(i):b(' <li role="presentation" class="'+a.widgetBaseClass+"-group "+k+(d[g].parentOptGroup.attr("disabled")?" "+this.namespace+
'-state-disabled" aria-disabled="true"':'"')+'><span class="'+a.widgetBaseClass+'-group-label">'+d[g].parentOptGroup.attr("label")+"</span><ul></ul></li> ").appendTo(this.list).find("ul").append(i)}else i.appendTo(this.list);if(c.icons)for(var e in c.icons)if(i.is(c.icons[e].find)){i.data("optionClasses",d[g].classes+" "+a.widgetBaseClass+"-hasIcon").addClass(a.widgetBaseClass+"-hasIcon");k=c.icons[e].icon||"";i.find("a:eq(0)").prepend('<span class="'+a.widgetBaseClass+"-item-icon ui-icon "+k+'"></span>');
...
this.selectmenuIcon.toggleClass("ui-icon-triangle-1-s",g).toggleClass("ui-icon-triangle-2-n-s",!g);c.style=="dropdown"?this.list.width(c.menuWidth?c.menuWidth:c.width):this.list.width(c.menuWidth?c.menuWidth:c.width-c.handleWidth);this.list.css("height","auto");g=this.listWrap.height();e=b(window).height();e=c.maxHeight?Math.min(c.maxHeight,e):e/3;g>e&&this.list.height(e);this._optionLis=this.list.find("li:not(."+a.widgetBaseClass+"-group)");this.element.attr("disabled")?this.disable():this.enable();
...
c){if(!this.options.disabled){var e=parseInt(this._selectedOptionLi().data("index")||0,10)+a;if(e<0)e=0;if(e>this._optionLis.size()-1)e=this._optionLis.size()-1;if(e===c)return false;if(this._optionLis.eq(e).hasClass(this.namespace+"-state-disabled")){a>0?++a:--a;this._moveSelection(a,e)}else this._optionLis.eq(e).trigger("mouseover").trigger("mouseup")}},_moveFocus:function(a,c){var e=isNaN(a)?parseInt(this._optionLis.filter(a).data("index"),10):parseInt(this._focusedOptionLi().data("index")||0,
10)+a;if(e<0)e=0;if(e>this._optionLis.size()-1)e=this._optionLis.size()-1;if(e===c)return false;c=this.widgetBaseClass+"-item-"+Math.round(Math.random()*1E3);this._focusedOptionLi().find("a:eq(0)").attr("id","");if(this._optionLis.eq(e).hasClass(this.namespace+"-state-disabled")){a>0?++a:--a;this._moveFocus(a,e)}else this._optionLis.eq(e).find("a:eq(0)").attr("id",c).focus();this.list.attr("aria-activedescendant",c)},_scrollPage:function(a){var c=Math.floor(this.list.outerHeight()/this._optionLis.first().outerHeight());
...
1E3);this.list.find("."+this.widgetBaseClass+"-item-selected").removeClass(this.widgetBaseClass+"-item-selected"+a).find("a").attr("aria-selected","false").attr("id","");this._selectedOptionLi().addClass(this.widgetBaseClass+"-item-selected"+a).find("a").attr("aria-selected","true").attr("id",c);a=this.newelement.data("optionClasses")?this.newelement.data("optionClasses"):"";var e=this._selectedOptionLi().data("optionClasses")?this._selectedOptionLi().data("optionClasses"):"";this.newelement.removeClass(a).data("optionClasses",
...
(function(b,a,c){function d(k){return k}function h(k){return decodeURIComponent(k.replace(g," "))}var g=/\+/g,i=b.cookie=function(k,e,f){if(e!==c){f=b.extend({},i.defaults,f);if(e===null)f.expires=-1;if(typeof f.expires==="number"){var j=f.expires,l=f.expires=new Date;l.setDate(l.getDate()+j)}e=i.json?JSON.stringify(e):String(e);return a.cookie=[encodeURIComponent(k),"=",i.raw?e:encodeURIComponent(e),f.expires?"; expires="+f.expires.toUTCString():"",f.path?"; path="+f.path:"",f.domain?"; domain="+
f.domain:"",f.secure?"; secure":""].join("")}e=i.raw?d:h;f=a.cookie.split("; ");j=0;for(l=f.length;j<l;j++){var n=f[j].split("=");if(e(n.shift())===k){k=e(n.join("="));return i.json?JSON.parse(k):k}}return null};i.defaults={};b.removeCookie=function(k,e){if(b.cookie(k)!==null){b.cookie(k,null,e);return true}return false}})(jQuery,document);
